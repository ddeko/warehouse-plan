...
export function FeedbackModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Could not reach the feedback service.')
      setBusy(false)
    }
  }
      ...
      <div className="grid gap-3">
        ...
        {!feedbackConfigured && (
          <p className="rounded-lg border px-3 py-2 text-[12px] leading-relaxed"
            ...
            No feedback endpoint in this build. Deploy <code className="mono">apps-script/feedback.gs</code> and put its
            URL in <code className="mono">VITE_FEEDBACK_URL</code>, then rebuild.
          </p>
        )}

        <SelectField label="What is this about?" value={kind} onChange={setKind} options={FEEDBACK_KINDS} />

        <Field
          label="Details"
          hint={`${message.length} / ${MAX_MESSAGE} characters`}
        >
          <textarea
            ...
            placeholder="What happened, or what would you like to see? Steps that reproduce a bug are gold."
            value={message}
        ...
        <TextField
          ...
          hint="Only needed if you want a reply. Your data stays in your browser either way."
        />
